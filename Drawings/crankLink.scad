...
echo(str("rHole=",rHole));
  difference() {
    union() {
      hull() {
        sphereSection(radA+3,NodeHeight/2,ar=0.7,off=0,fn=64);
        translate([dFlat*1.5,0,NodeHeight/4]) sphere(NodeHeight/4+.05,$fn=12);
      }

      translate([len,0,0]) 
        linkNode(rHole+2,NodeHeight,ar=1.3,off=-.1,dx=-8*rHole);
